...
我们可以找出不少重复的片段. 我们规定, 只要有两个以上的字符与前文重复, 就使用重复记号 `<d,l>` 代替它. `<d,l>` 表示这个位置的字符串等价与 `d` 个字符前, 长度为 `l` 的字符串. 于是上面的内容就可以表示为:
...
顺序查找过于缓慢, 我们可以使用哈希表来提升效率. 我们使用一个哈希表把一个长度为三的字符串映射成一个列表, 包含这个字符串所有出现过的位置. 压缩时从头到尾扫描字符串, 假设接下来的三个字符为 `XYZ`, 首先检查字符串 `XYZ` 是否在哈希表中. 如果不在, 则原样输出字符 `X` 并移动到下一个字符. 否则把当前位置开始的字符串与所有 `XYZ` 出现过的位置开始的字符串相比较, 寻找出一个长度为 N 的最长的匹配并输出重复记号, 然后把 `XYZ` 与当前位置插入到哈希表中. 接着向后移动 N 个字符.
...
为了能够高效地储存 Huffman 编码表, DEFLATE 算法中的使用的 Huffman 编码有以下两个附加规则:

1. 相同长度的编码应当从小到大排列. 也就是说 3.1 节中 `0101` 排在 `0100` 之前是不允许的.
2. 较短的编码应当小于较长的编码. 也就是说 3.1 节中的 `00` 和 `1` 是不允许的, 因为前者数值小于后者, 长度却大于后者.
