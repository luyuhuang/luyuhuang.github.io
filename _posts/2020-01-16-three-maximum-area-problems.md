...
注意到一个柱子上储水的最大值取决于**两边柱子最大高度的较小值**.
```python
def trapping_rain_water(heights):
    ans = 0
    i, j = 0, len(heights) - 1
...
与之前的按列求储水量不同, 我们还可以换种思路: 按行求储水量. 对于每个柱子, 都分别在它的左右两边寻找第一个高度比它大的柱子, 然后取其中较小的高度再根据柱子间的距离计算出储水量, 最后把所有的储水量加起来便是问题的解.

对于每个柱子, 如何求出它左右两边第一个高度比它大的柱子呢? 如果我们使用最朴素的做法, 即遍历所有的柱子, 分别向左右两边寻找高度比它大的柱子, 那么时间复杂度为 $\mathrm{O}(n^2)$. 这显然是不合适的.

这里我们采用一种叫做**单调栈**的技巧. 所谓单调栈就是栈中的元素始终保持递增或者递减. 在这道题中, 我们在栈中保存每个柱子的下标, 使栈中柱子的高度保持递减. 为了做到这一点, 我们在每次入栈时都与栈顶相比较, 如果要入栈的柱子高度比栈顶小则直接入栈, 否则不断地弹出栈顶, 直到栈为空或栈顶柱子的高度大于要入栈柱子的高度.

想想看, 当栈弹出的时候会发生什么呢? 因为栈中柱子的高度是单调递减的, 所以对于刚刚出栈的那个柱子而言, <u>左边第一个高度大于于它的柱子恰恰就是栈顶的柱子</u>; 而<u>右边第一个高度大于它的柱子恰恰就是将要入栈的柱子</u>. 这样一来就能完美地解决问题.
...
"为每个柱子寻找左右两边第一个高度小于它的柱子" 听起来是不是有点熟悉? 不错, 跟接雨水的思路二一样, 使用单调栈即可. 不同的是这道题我们要用递增的单调栈.
