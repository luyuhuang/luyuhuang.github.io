...
三元组 `(i, j, k)` 中的 `n` 至关重要. 想象一个格子填了 1, 然后在后续的迭代中走入了 "死胡同", 回溯回来了, 这时候应该把这个格子上的数字 1 删掉, 然后重新从 2 开始选择数字. 因此每次填数字时都应该更新 `n`, 每次选择数字时都应该从 `n + 1` 开始选择, `n` 初始应该为 0. 此外我们还可以通过判断 `n` 是否为 0 来判断这个格子是迭代而来, 还是回溯而来.
